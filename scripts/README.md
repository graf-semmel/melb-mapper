...
### `copy-geo-data.sh`
Copies processed suburb and bounds JSON files from `scripts/geo` to `public/geo` for deployment.
...
Copies processed suburb and bounds JSON files from `scripts/geo` to `public/geo`, excluding raw OSM files.
...
**What it copies:**
- All `*.suburbs.json` files (excluding `*.osm.json`)
- All `*.bounds.json` files
- Only top-level files in `scripts/geo/` (excludes subdirectories like `perth/`)
- Creates destination directory as needed
...
- **City of Darwin required instead of Darwin:** Using the fetch-suburbs.sh script to get suburbs for "Darwin" may not work as expected. Instead, use "City of Darwin" to ensure accurate geocoding and data retrieval.
